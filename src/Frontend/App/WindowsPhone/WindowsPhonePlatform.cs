...
{
    /// <summary>
    /// Platform implementation for Windows Phone 8
    /// </summary>
    internal class WindowsPhonePlatform : IPlatform
    {
        /// <summary>
        ...
        public bool FileExists(string filename)
        {
            try
            {
                ...
                if (filename.StartsWith(localFolder))
                {
                    string localFilename =
                        filename.Substring(localFolder.Length)
                        .TrimStart('\\');

                    ApplicationData.Current.LocalFolder.GetFileAsync(localFilename).GetResults();
                }
                else
                {
                    var folder = StorageFolder.GetFolderFromPathAsync(Path.GetDirectoryName(filename)).GetResults();

                    folder.GetFileAsync(Path.GetFileName(filename)).GetResults();
                }
            }
            catch (InvalidOperationException)
            {
                // thrown when app doesn't have permission to access the storage folder
                return false;
            }
